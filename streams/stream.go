...
var (
	KEY    = "u0JLcZ7coZ31TO0vEUz4Dmgtc"
	SECRET = "KJq647pKqf6ZTcJghI9jKML5Je9eBwYthffa1EFpqTKEjBsQOH"
)
...
func InitiateStream() {
	...
	for tweet := range api.GetMessages() {
		...
		userid, err := strconv.Atoi(result.Data.AuthorID)
		if err != nil {
			log.Fatalf("error getting user id %v", err)
		}
		user := twitter.GetUserById(userid)

		//fmt.Println(result.Data)

		TweetDeck.Text = result.Data.Text
		TweetDeck.ID = result.Data.ID
		TweetDeck.CreatedAt = result.Data.CreatedAt
		TweetDeck.AuthorName = user.Data.Name
		TweetDeck.Username = "@" + user.Data.Username
		TweetDeck.Profile_image_url = user.Data.Profile_image_url
		fmt.Println(TweetDeck)
	}
